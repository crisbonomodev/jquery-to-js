...
switch (category) {
  case 'action': return findByMovie(actionList,id);
  case 'drama': return findByMovie(dramaList,id);
  default: return findByMovie(animationList,id);
}
}
...
$form.addEventListener('submit',(async(event)=> {
  ...
  const {//desestructuramos la variable para guardar solo la info que hay en el array movies dentro de data
    data: {movies}
  } = await getData(`${API}list_movies.json?limit=1&query_term=${formData.get('name')}`);
...
async function cacheExists(category) {
  ...
  {
    const {data:{movies}} = await getData(`${API}list_movies.json?genre=${category}`);
